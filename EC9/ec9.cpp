...
class PathfindingAlgorithm {
  ...
  public:
    ...
    // Function to print the path over the maze (common to all algorithms)
    void printPath(const std::vector<std::pair<int, int>> &path) {
        ...
        // Store the path in an unordered set for efficient lookup
        std::unordered_set<std::pair<int, int>, std::hash<int>,
                           std::equal_to<int>>
            pathSet(path.begin(), path.end());
        ...
        for (int i = 0; i < maze.getRows(); ++i) {
            for (int j = 0; j < maze.getCols(); ++j) {
                if (pathSet.find({i, j}) != pathSet.end()) {
                    ...
                    std::cout << maze.getCell(i, j) << " "; // Normal cell
                }
            }
            std::cout << std::endl;
        }
...
class Dijkstra : public PathfindingAlgorithm {
  public:
    ...
    void findPath() override {
        auto start_time =
            std::chrono::high_resolution_clock::now(); // Start time measurement
        ...
        auto end_time =
            std::chrono::high_resolution_clock::now(); // End time measurement
        std::chrono::duration<double, std::milli> duration =
            end_time - start_time; // Calculate duration in milliseconds
...
// Function to demonstrate the pathfinding algorithms
void demonstratePathfinding() {
    Maze maze(5, 5);
    maze.setCell(0, 0, 0);
    maze.setCell(0, 1, 1); // Obstacle
    maze.setCell(0, 2, 0);
    maze.setCell(0, 3, 0);
    maze.setCell(0, 4, 0);
    maze.setCell(1, 0, 0);
    maze.setCell(1, 1, 0);
    maze.setCell(1, 2, 1); // Obstacle
    maze.setCell(1, 3, 0);
    maze.setCell(1, 4, 0);
    maze.setCell(2, 0, 0);
    maze.setCell(2, 1, 0);
    maze.setCell(2, 2, 0);
    maze.setCell(2, 3, 1); // Obstacle
    maze.setCell(2, 4, 0);
    maze.setCell(3, 0, 0);
    maze.setCell(3, 1, 1); // Obstacle
    maze.setCell(3, 2, 0);
    maze.setCell(3, 3, 0);
    maze.setCell(3, 4, 0);
    maze.setCell(4, 0, 0);
    maze.setCell(4, 1, 0);
    maze.setCell(4, 2, 1); // Obstacle
    maze.setCell(4, 3, 0);
    maze.setCell(4, 4, 0);
